...
#if ANDROID
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), Path.GetFileName(filePath));
            if (!File.Exists(path))
            {
                using (var src = TitleContainer.OpenStream(filePath))
